...
window.addEventListener(
    ...
    function (event) {
        ...
        const incidentId =
            event.detail?.incidentId;

        const vehicleId =
            event.detail?.vehicleId;
        ...
        /*
           Aktuelles Fahrzeugmenü komplett
           neu aufbauen.
        */
        const content =
            vehicleMenuOverlay?.querySelector(
...
function renderMeasureDetails(
    ...
    backButton.className = "vehicle-action-back";
    backButton.textContent = "‹ Maßnahmen";
...
function renderMeasureGroupDetails(
    container,
    incident,
    vehicle,
    measureKey,
...
item.addEventListener(
    ...
    return;
        ...
        const dropTarget =
            target.closest(
                ".incident-section, .incident-section-available"
            );


        document
            .querySelectorAll(
                ".incident-section.drag-over, .incident-section-available.drag-over"
            )
            .forEach(
                function (element) {
...
function renderProcedureControl(
    ...
    */
    ...
    procedure &&
    procedure.waterConsumptionPerMinute !== undefined &&
    hasWaterTank(vehicle)
) {

    renderWaterSupplyControl(
        container,
        incident,
        vehicle
    );
...
/* =========================================================
   SCHAUMMITTEL-ANZEIGE BEI BRANDBEKÄMPFUNG
========================================================= */

if (
    measureKey === "brandbekaempfung" &&
    procedureKey === "schaumrohr" &&
    vehicle.equipment &&
    vehicle.equipment.foamTank
) {
    ...
    foamAmount.className =
        "foam-tank-amount";


    function updateFoamDisplay() {
        ...
        foamAmount.textContent =
            current.toLocaleString("de-DE") +
            " / " +
            capacity.toLocaleString("de-DE") +
            " l";
    ...
    /*
       Während der Maßnahme regelmäßig aktualisieren.
    */
    const foamUpdateInterval =
        setInterval(
            function () {

                /*
                   Wenn das Element nicht mehr
                   im Dokument vorhanden ist,
                   Intervall beenden.
                */
                if (
                    !document.body.contains(
    ...
    if (
    measureKey === "brandbekaempfung" &&
    groupKey === "wasserversorgung" &&
    procedureKey === "wasserAnAnderesLF"
    (
        hasWaterTank(vehicle)
    )
) {

    renderWaterTransferControl(
        container,
        incident,
        vehicle
    );
...
/* =========================================================
   WASSER AN ANDERES LF
========================================================= */

function renderWaterTransferControl(
    container,
    incident,
    vehicle
) {
    ...
    /*
       Alle Fahrzeuge verwenden, die diesem
       Einsatz tatsächlich zugeordnet sind
       und sich vor Ort / auf Anfahrt befinden.
    */
        ...
        .filter(
            function (targetVehicle) {
                ...
                */
                ...
                /*
                   Nur Fahrzeuge mit Wassertank
                   können Wasser aufnehmen.
                */
                if (
                    !hasWaterTank(
                        targetVehicle
    ...
    if (
        ...
        defaultOption.textContent =
            "Keine anderen Fahrzeuge mit Wassertank.";
...
function renderWaterSupplyControl(
    container,
    incident,
    vehicle
) {
...
function createWaterSourceButton(
    container,
    incident,
    vehicle,
    source,
    ...
    if (
        ...
        if (
            ...
            if (
                ...
                alert(
                    "Bitte zuerst ein Zielfahrzeug auswählen."
                );
...
function getProcedure(
    ...
    */
    ...
    if (
        ...
        measure.measures[groupKey].procedures
    ) {
